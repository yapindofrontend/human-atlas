...
import {SYSTEMS,bodyBounds,partInArea,partIsVisible,partRegion,type Atlas,type Part,type SceneState} from './anatomy';
...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  camera.position.set(1.4,1.05,3.6);controls.target.set(0,.85,0);controls.enableDamping=true;controls.dampingFactor=.085;controls.zoomToCursor=true;controls.minDistance=.07;controls.maxDistance=40;controls.maxPolarAngle=Math.PI*.96;controls.addEventListener('change',()=>{dirty=true;});
  ...
  const fit=(view:string,extent=0)=>{
   ...
   if((area||region)&&extent<.1){
    const dir=view==='front'?new T.Vector3(0,.02,1):view==='back'?new T.Vector3(0,.02,-1):view==='side'?new T.Vector3(1,.02,0):new T.Vector3(.35,.06,1).normalize();
   ...
   const mobile=el.clientWidth<768,portrait=mobile&&el.clientHeight>=el.clientWidth,normalDistance=mobile?Math.max(4.5,1.8*el.clientHeight/Math.max(160,el.clientHeight-(portrait?440:350))/(2*Math.tan(T.MathUtils.degToRad(camera.fov/2)))):4;
   ...
   controls.target.set(0,extent>.1||mobile?.85:.68,0);camera.position.copy(controls.target).addScaledVector(direction,distance);controls.update();dirty=true;
  };
  ...
  const animate=()=>{
   ...
   const changed=lastState?.visible!==s.visible||lastState?.selected!==s.selected||lastState?.isolate!==s.isolate||lastState?.breastView!==s.breastView||lastState?.region!==s.region||lastState?.area!==s.area;
   ...
   if(changed||moving||lastExtent<0){
    ...
    const shown=(p:Part,i:number)=>{if(!partIsVisible(p,s,visibilityLookups))return false;if(s.isolate)return true;if(visibilityLookups.selected.has(p.id))return true;if(s.area)return partInArea(p,s.area,body);if(s.region&&regions[i]!==s.region)return false;return true;};
   ...
   if(moving&&!s.isolate&&amount>.45)fit(amount>.5?'front':s.view,Math.max(0,(amount-.3)/.7));
