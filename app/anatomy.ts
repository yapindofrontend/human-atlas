...
export const SYSTEMS: { id: SystemId; name: string; color: string; description: string }[] = [
  ...
  },
  {
    ...
    color: "#d8bd82",
    description:
      "Breast tissue includes adipose tissue, mammary glands, ducts, and connective supports. It lies over the pectoral muscles and does not act as a skeletal muscle to move the shoulder. Colors distinguish tissue types; they do not show activation or fiber direction.",
  },
  {
    id: "pregnancy",
    ...
    color: "#b88380",
    description:
      "Placenta and umbilical reference structures, separate from the default adult anatomy.",
  },
  {
    id: "connective",
...
export interface Part {
  ...
  provenance?: { source: string; sourceId: string; adaptation: string };
}
...
export const AREAS: { id: AreaId; name: string; regions: RegionId[]; match: RegExp }[] = [
  {
    ...
    regions: ["head-neck"],
    match:
      /(choroid|cornea|iris|sclera|lens|eyeball|optic nerve|optic chiasm|optic tract|ophthalmic nerve|lacrimal nerve|oculomotor|trochlear nerve|nasociliary|ciliary ganglion|ciliary nerve|frontal nerve|infratrochlear|supra-orbital nerve|supratrochlear|check ligament|levator palpebrae|trochlea of (left|right) superior oblique|(left|right) (inferior|superior|lateral|medial) rectus|(left|right) (inferior|superior) oblique)/i,
  },
  {
    id: "willis",
    ...
    regions: ["head-neck"],
    match:
      /(anterior communicating|posterior communicating|precommunicating part|cerebral arterial circle|basilar artery|internal carotid|vertebral artery|postcommunicating)/i,
  },
  {
    id: "brainstem",
    name: "Brainstem",
    regions: ["head-neck"],
    match:
      /(medulla oblongata|^pons$|midbrain|cerebral aqueduct|(inferior|superior) colliculus|peduncle of midbrain|interpeduncular)/i,
  },
  {
    id: "larynx",
    name: "Larynx",
    regions: ["head-neck"],
    match:
      /(crico-arytenoid|thyro-arytenoid|arytenoid|epiglot|vocalis|cricoid|thyroid cartilage|hyoid bone|cricothyroid|vocal ligament|conus elasticus|aryepiglottic)/i,
  },
  {
    id: "heart",
    name: "Heart",
    regions: ["torso"],
    match:
      /(cavity of (left|right) (ventricle|atrium)|mitral|tricuspid|(aortic|pulmonary) valve|cusp of|wall of ventricle|papillary muscle|coronary (artery|sinus)|ascending aorta|arch of aorta)/i,
  },
  {
    id: "lung-root",
    name: "Lung roots",
    regions: ["torso"],
    match: /(main bronchus|pulmonary arter|pulmonary vein)/i,
  },
  {
    id: "porta",
    name: "Porta hepatis",
    regions: ["abdomen"],
    match:
      /(portal vein|hepatic artery|bile duct|cystic duct|hepatic duct|gallbladder|caudate lobe of liver)/i,
  },
  {
    id: "celiac",
    name: "Celiac trunk",
    regions: ["abdomen"],
    match:
      /(celiac trunk|celiac artery|splenic artery|left gastric|common hepatic artery|hepatic artery proper)/i,
  },
  {
    id: "kidneys",
    name: "Kidneys",
    regions: ["abdomen"],
    match: /(kidney|adrenal|suprarenal|renal arter|renal vein)/i,
  },
  {
    id: "brachial-plexus",
    name: "Brachial plexus",
    regions: ["arm", "head-neck"],
    match:
      /(scalenus|subclavius|subclavian artery|subclavian vein|axillary artery|axillary vein|clavicle|thoraco-acromial|circumflex humeral|circumflex scapular|subscapular artery|subscapular vein|thoracodorsal artery|thoracodorsal vein|vertebral artery)/i,
  },
  {
    id: "axilla",
    name: "Axilla",
    regions: ["arm"],
    match:
      /(axillary (artery|vein)|subscapular|thoracodorsal|circumflex (humeral|scapular)|latissimus|teres major|teres minor|pectoralis minor)/i,
  },
  {
    id: "cubital",
    name: "Cubital fossa",
    regions: ["arm"],
    match:
      /(median cubital|brachialis|anconeus|ulnar recurrent|radial recurrent|ulnar collateral|recurrent interosseous)/i,
  },
  {
    id: "wrist",
    name: "Wrist",
    regions: ["arm"],
    match:
      /(flexor retinaculum of .+ wrist|scaphoid|lunate|triquetral|pisiform|hamate|capitate|trapezium|trapezoid)/i,
  },
  {
    id: "hand",
    name: "Hand",
    regions: ["arm"],
    match:
      /(finger|thumb|pollic|metacar|thenar|palmar digital|interossei of (left|right) hand|lumbricals of (left|right) hand|opponens|abductor digiti minimi of (left|right) hand|dorsal venous network of .+ hand)/i,
  },
  {
    id: "pelvic-viscera",
    name: "Pelvic viscera",
    regions: ["pelvis"],
    match: /(urinary bladder|prostate|rectum|seminal|deferent|epididymis|testis)/i,
  },
  {
    id: "popliteal",
    name: "Popliteal fossa",
    regions: ["legs"],
    match: /(popliteal|popliteus|genicular)/i,
  },
  {
    id: "foot",
    name: "Foot",
    regions: ["legs"],
    match:
      /\b(foot|toe|talus|calcaneus|metatars|cuneiform|navicular|cuboid|plantar|hallucis|dorsal venous arch of .+ foot)/i,
  },
];
...
const ARM_LATERAL = 0.22;
const SHOULDER_LATERAL = 0.165;
...
export function partRegion(part: Part, body: [number[], number[]]): RegionId {
  ...
  const ny = size[1] > 0 ? (cy - min[1]) / size[1] : 0;
  const lat = size[0] > 0 ? Math.abs((cx - min[0]) / size[0] - 0.5) : 0;
  if (ny >= REGION_Y.head) return "head-neck";
  if (
    ny >= REGION_Y.arm &&
    ny < REGION_Y.head &&
    (lat >= ARM_LATERAL || (ny >= REGION_Y.shoulder && lat >= SHOULDER_LATERAL))
  )
    return "arm";
...
export function isPartVisible(
  part: Part,
  state: Pick<SceneState, "visible" | "selected" | "isolate" | "region" | "area">,
  body: [number[], number[]]
) {
...
export const EXPLANATIONS: Record<string, string> = {
  "adipose tissue of left breast":
    "Fat contributes to breast volume and contour, surrounding the mammary glands and ducts. It lies superficial to the pectoral muscles and does not contract to move the shoulder.",
  "adipose tissue of right breast":
    "Fat contributes to breast volume and contour, surrounding the mammary glands and ducts. It lies superficial to the pectoral muscles and does not contract to move the shoulder.",
  heart:
  ...
  kidney:
    "A paired organ at the back of the abdomen. Its nephrons filter blood, reabsorb what the body needs, and produce urine that drains into the ureter.",
  ureter:
    "A muscular tube carrying urine from the kidney to the bladder. Waves of contraction move urine along it rather than gravity alone.",
  urethra: "The final passage carrying urine from the bladder out of the body.",
  "right lung":
    "The right lung has three lobes. Air arriving through the bronchial tree reaches alveoli, where oxygen and carbon dioxide exchange with the blood.",
  "left lung":
    "The left lung has two lobes, leaving room for the heart. Its bronchial tree ends in alveoli where gas exchange takes place.",
  bronchus:
    "The airways branching from the trachea into each lung, dividing repeatedly into smaller passages that end at the alveoli.",
  esophagus:
    "A muscular tube from the pharynx to the stomach. Coordinated waves of contraction carry swallowed food downward.",
  "small intestine":
    "The long, coiled segment where most chemical digestion and nutrient absorption occur, beginning at the duodenum.",
  "large intestine":
    "The final segment of the digestive tract. It absorbs water and salts and forms and stores the residue for elimination.",
  duodenum:
    "The first part of the small intestine. Bile and pancreatic enzymes enter here to continue digestion.",
  gallbladder:
    "A small sac beneath the liver that concentrates and stores bile, releasing it into the duodenum.",
  ...
  testis:
    "A paired organ producing sperm and testosterone. Its seminiferous tubules are the site of sperm formation.",
  epididymis: "A coiled duct on the testis where sperm mature and are stored before transport.",
  "seminal vesicle":
    "A paired gland contributing much of the fluid volume of semen, including sugars that support sperm.",
  prostate:
    "A gland surrounding the start of the urethra. Its secretions form part of the seminal fluid.",
};
/** Focused study modes. */
...
export const MODES: Mode[] = [
  {
    ...
    systems: ["cardiac"],
    focus: "FMA7088",
    summary: "Chambers, valves and septa of the four-chambered pump.",
    tour: ["FMA7088"],
  },
  {
    id: "respiratory",
    ...
    systems: ["respiratory"],
    focus: "FMA7394",
    summary: "The airway from the trachea through the bronchial tree into both lungs.",
    tour: ["FMA7394", "FMA7409", "FMA7309", "FMA7310"],
  },
  {
    id: "digestive",
    name: "Digestive",
    systems: ["digestive"],
    focus: "FMA7148",
    summary: "The tract from esophagus to rectum, with the liver and pancreas.",
    tour: [
      "FMA7131",
      "FMA7148",
      "FMA7206",
  ...
  },
  {
    ...
    systems: ["urinary"],
    focus: "FMA7203",
    summary: "Kidneys, ureters, bladder and urethra as one drainage path.",
    tour: ["FMA7203", "FMA9704", "FMA15900", "FMA19667"],
  },
  {
    id: "reproductive",
    name: "Reproductive",
    ...
    focus: "FMA7210",
    summary: "Sperm production, maturation and transport, and the accessory glands.",
    tour: ["FMA7210", "FMA18255", "FMA19386", "FMA9600"],
  },
];
export function explanation(name: string, system: SystemId, sex: "male" | "female" = "male") {
  return (
    EXPLANATIONS[name.toLowerCase()] ??
    (sex === "female" && system === "reproductive"
      ? "Female reproductive structures represented in this reference include the ovaries, uterine tubes, uterus, cervix, vagina, and supporting tissues."
...
export function partIsVisible(
  ...
  const breast =
    part.system === "mammary" ||
    (part.system === "integumentary" && part.id.startsWith("VH_F_") && part.id !== "VH_F_skin");
